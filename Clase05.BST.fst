...
let rec all_lt_not_in_tree (x:int) (t:bst) (y:int) : Lemma (requires x > y /\ all_lt y t) 
                                                           (ensures ~ (in_tree x t)) =
  match t with
  | L -> ()
  | N (l,_,r) -> all_lt_not_in_tree x l y;
                 all_lt_not_in_tree x r y 

let rec all_gt_not_in_tree (x:int) (t:bst) (y:int) : Lemma (requires x < y /\ all_gt y t)
                                                           (ensures ~ (in_tree x t)) =
  match t with
  | L -> ()
  | N (l,_,r) -> all_gt_not_in_tree x l y;
