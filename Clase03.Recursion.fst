...
// Decrece [5, 0] -> [4,1] -> [4, 0] -> [3,1] -> [3,0] -> [2,1] ...
...
let rec sum (xs:list int) : int =
  ...
  (* La acepta porque como .tl xs es un subtermino de xs, cumple con la propiedad de predecesor *)
  if Nil? xs
  then 0
...
let rec sum' (xs:list int) : Tot int =
  ...
  (* No se acepta porque el resultado de (init xs) no devuelve un 
     subtermino de xs a partir de sus constructores. Si termina *)
  if Nil? xs
  then 0
